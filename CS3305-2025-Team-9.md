...
database. Multiple devices can be logged into a single account at once.
...
**Include pic for manually adding ingredients here**
**JACK PLEASE !!!!!!!!!!!!!!!!!**
...
#### 3.9.1 User Details
The user details section dynamically displays account information from Firebase such as username, 
email and household ID. This information is dynamically updated using LiveData and ViewModel whenever 
changes are made. 

The user's profile picture is also displayed. This profile picture is selected from a predefined
set of nine options stored as XML drawables. The selected profile picture is saved and displayed with 
SharedPreferences, without the need of a reload. The selection also persists across app restarts.
The selection process is done through a popup window using GridLayout.
...
The Edit Profile activity allows users to modify their display name, email and profile picture. 
...
all changes I made to their files, be it UI or functional. To make thing run more efficiently, I put these features 
