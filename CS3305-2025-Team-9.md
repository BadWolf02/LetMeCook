...
When switching between different acctivities intents are used in order to go from one activity to another and pass relevant data from one activity to another.
This method is used to get from the sign up activity to the login activity on account creation as shown in the bellow image. 
It is also used from the inventory in order to launch the activity handeling the barcode scanning when adding items,
in add recipe in order to handel uploading an image from the users gallery,
when  switching from the user fragment to viewing the invites, when switching to the edit profile activity, when signing out and switching back to the login activity
as well as to go from the home page to the view recipe activity.
