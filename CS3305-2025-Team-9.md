...
When switching between different activities intents are used in order to go from one activity to another and pass relevant data from one activity to another.
...
It is also used from the inventory in order to launch the activity handling the barcode scanning when adding items,
